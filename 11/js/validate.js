function validate(imageUploadForm){
  ...
  function validateHashtags(hashtagsString) {
    ...
    const hashtags = hashtagsString.trim().split(/\s+/).filter((hashtag) => hashtag !== '');
  ...
  pristine.addValidator(hashtagField, validateHashtags, 'Некорректные хэштеги');
  pristine.addValidator(commentField, validateComment, 'Максимум 140 символов');
